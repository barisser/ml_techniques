...
def shorten(x, max, border, sharpness):
...
class SimpleNN(nn.Module):
    def __init__(self):
        ...
        self.max = 1000
        self.w1 = nn.Parameter(torch.tensor(0.35))
        self.w2 = nn.Parameter(torch.tensor(0.05))
        self.target_sum = torch.tensor(0.4)
    ...
    def forward(self, x):
        ...
        x2 = shorten(x1, self.max, self.w1, 2)
        x3 = torch.relu(self.fc2(x2))

        x4 = shorten(x3, self.max, self.w2, 2)
        x5 = self.fc3(x4)
...
model.train_model(train_loader, 25)
